...
st.markdown("Analyze your Driver & Irons, upload gear photos and swing video, audit your setup, and receive your **Top 5 Prescribed Builds**.")
st.markdown("---")
...
if st.button("Generate Full Diagnostic & Top 5 Recommendations", type="primary"):
    ...
    with col_res1:
        ...
        # Driver Head Verdict
        if keep_driver_head:
            st.markdown(f"**Head Verdict:** ✅ **KEEP YOUR `{curr_driver_head}`** — This head profile matches your delivery profile perfectly. No need to spend money replacing it!")
        else:
            st.markdown(f"**Head Verdict:** 🔄 **UPGRADE RECOMMENDED** — Your current head is fighting your delivery. Look for a **{d_head_key}** head.")
        ...
        if keep_driver_shaft:
            ...
            st.markdown(f"**Shaft Verdict:** ⚠️ **RESHAFTOVERRIDE NEEDED** — Reshafting your head with a **{d_shaft_key.split('(')[0]}** will immediately stabilize your face angle and tighten dispersion.")
    ...
    with col_res2:
        ...
        # Iron Head Verdict
        if keep_iron_heads:
            st.markdown(f"**Iron Head Verdict:** ✅ **KEEP YOUR `{curr_iron_heads}`** — The forgiveness and offset are well matched to your handicap.")
        else:
            st.markdown(f"**Iron Head Verdict:** 🔄 **UPGRADE RECOMMENDED** — Consider transitioning to **{i_head_key}** irons.")
