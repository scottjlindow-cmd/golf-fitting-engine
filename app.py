...
if st.button("Generate Full Diagnostic & Top 5 Recommendations", type="primary"):
    ...
    with col_res1:
        ...
        st.markdown(f"**Current Driver In Bag:** `{curr_driver_head}` paired with `{curr_driver_shaft}`")
        st.markdown(f"**Optimal Setup:** Upgrade/adjust to a **{d_head_key}** head with a **{d_shaft_key.split('(')[0]}** shaft.")
    
    with col_res2:
        st.info(f"**Physical Build Spec:** Length: `{length_spec}` | Lie Angle: `{base_lie}` | Grips: `{grip_rec}`")
        st.markdown(f"**Current Irons In Bag:** `{curr_iron_heads}` paired with `{curr_iron_shafts}`")
        st.markdown(f"**Optimal Setup:** Target **{i_head_key}** heads with **{i_shaft_key}**.")
