...
class FittingReportPDF(FPDF):
    def header(self):
        ...
        self.cell(0, 8, 'COMPLETE GOLF GEAR DIAGNOSTIC & PRESCRIPTION REPORT', border=False, align='C', new_x="LMARGIN", new_y="NEXT")
        self.set_font('Helvetica', 'I', 8)
        self.cell(0, 5, 'Driver & Iron Performance Optimization Blueprint', border=False, align='C', new_x="LMARGIN", new_y="NEXT")
        self.ln(4)
...
# ==========================================
# STEP 2: DRIVER DELIVERY & MISS PATTERNS
# ==========================================
st.header("Step 2: Driver Trajectory & Delivery Audit")
...
if st.button("Generate Full Diagnostic & Top 5 Recommendations", type="primary"):
    ...
    # 2. Driver Diagnostic Logic
    if "High Slice" in driver_miss or "Push-Fade" in driver_miss:
        d_root = "Open dynamic face relative to path; insufficient heel closure and excess cut spin."
    ...
    elif "Snap-Hook" in driver_miss or "Ballooning" in driver_apex:
        d_root = "Forward shaft kick over-delivering dynamic loft with face closed early; high spin rate."
        d_head_key = "Low Spin / Forward CG"
        d_shaft_key = "Heavy / Low Launch / Low Spin (Stiff - X-Flex)"
    else:
        d_root = "Delivery baseline requires dynamic MOI stability and optimized launch spin."
    ...
    with col_res1:
        ...
        if curr_driver_head or curr_driver_shaft:
            ...
            st.markdown(f"**Status:** Adjust hosel sleeve or upgrade to a **{d_head_key}** head profile paired with a **{d_shaft_key.split('(')[0]}** shaft.")
    ...
    with col_res2:
        ...
        if curr_iron_heads or curr_iron_shafts:
            ...
            st.markdown(f"**Status:** Match build to **{i_head_key}** heads with **{i_shaft_key}**.")
    ...
    with st.form("lead_form"):
        ...
        if btn and u_email:
            st.success("Prescription generated successfully! Your PDF summary is ready.")
